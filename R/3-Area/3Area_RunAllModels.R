...
#data$srv_sel_by_year_indicator[projyears > 2015, 1] = 1
...
parameters$trans_srv_q = array(log(7000), dim =c(n_regions, length(unique(data$srv_q_by_year_indicator)), data$n_surveys))
...
## turn off initial-rec-devs
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("NoTagData", ifelse(AF_direct_ageing, "a", ""))
...
input_obs = plot_input_observations(data, region_key = region_key)
input_obs
...
######################
# Estimate regional recruitment devs
######################
scenario = paste0("NoTagData_regionaldevs", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
mle_lst = list(
  no_move = mle_report_1,
  fenske_movement = mle_report
)
ssbs = get_multiple_ssbs(mle_lst, run_labels = names(mle_lst), region_key = region_key, depletion = F)
srv_ndx = get_multiple_index_fits(mle_lst, run_labels = names(mle_lst), region_key = region_key)
ggplot(ssbs, aes(x = Year, y = SSB, col = label, linetype = label)) +
  geom_line(linewidth = 1.1) +
  facet_wrap(~Region)
ggplot(srv_ndx, aes(x = Year)) +
  geom_line(aes(y = Predicted,  col = label, linetype = label), linewidth = 1.1) +
  geom_point(aes(y = Observed,  col = "Observed"), size = 1.1) +
  facet_wrap(~Region)
...
######################
scenario = paste0("fenske_movement", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}

data$fixed_movement_matrix = fenske_move_matrix / 100
## Make AD Fun
mle_obj <- MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T, silent = T)
...
## assumes global recruitment deviations
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("TagData_01", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("TagData_01_alt", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
if(read_in_previous_model_data_and_params) {
  previous_scenario = paste0("TagData_01", ifelse(AF_direct_ageing, "a", ""))
  previous_fig_path = file.path(DIR$app_3A, previous_scenario)
...
## assumed to be Poisson
## and estimate movement
## assumes regional recruitment deviations
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("TagData_reg_rec", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
## tag-reporting rate decadal
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("TagData_decadal", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
tag_reporting_rate = c(1978, 1990, 2000, 2010)
## some pars to fix
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
...
######################
## Change tag-recovery likelihood 
##  to be Negative Binomial
## and estimate movement
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("TagData_01a", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_3A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
## Estimate-movement with regional recruitment
######################
# options(warn=0) ## if warnings are being set to errors
...
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   ...
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,  ## don't estimate the last one
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_report_time_block, # turn off
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement, # turn on and off to estimate
                                   est_prop_male_recruit = est_prop_male_recruit,
...
######################
## Model 04
## Have regional recruitment
## but constrain estimated recruit-devs to sum = 0 
######################
# options(warn=0) ## if warnings are being set to errors
...
## if starting from here re-load previous models data and parameters
if(T) {
  prev_scenario = paste0("Model_03", ifelse(AF_direct_ageing, "a", ""))
  prev_fig_path = file.path(DIR$app_3A, prev_scenario)
  data = readRDS(file.path(prev_fig_path, "data.RDS"))
  parameters = readRDS(file.path(prev_fig_path, "parameters.RDS"))
}
...
######################
## Model 05
## Same as 04, but reduce init_age_devs estimated from 28 -> 15
######################
# options(warn=0) ## if warnings are being set to errors
...
## if starting from here re-load previous models data and parameters
if(T) {
  prev_scenario = paste0("Model_04", ifelse(AF_direct_ageing, "a", ""))
  prev_fig_path = file.path(DIR$app_3A, prev_scenario)
  data = readRDS(file.path(prev_fig_path, "data.RDS"))
  parameters = readRDS(file.path(prev_fig_path, "parameters.RDS"))
}
## double check some switches
data$tag_likelihood
data$evaluate_tag_likelihood

## change init devs
data$n_init_rec_devs = 15
parameters$ln_init_rec_dev = rep(0, data$n_init_rec_devs)
...
## do an additional two Newton Raphson iterations to try and improve the fit.
## this will also check that the Hessian is well defined i.e. PD covariance. The Hessian
## is used to do the Newton Raphson iterations
try_improve = tryCatch(expr =
                         for(i in 1:2) {
                           g = as.numeric(mle_obj$gr(mle_spatial$par))
                           h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
