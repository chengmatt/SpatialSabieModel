...
design_survey_index_previous = readRDS(file = file.path("Data", "Survey", "survey_ndx_5_area_combined_countries.RDS"))
design_survey_index_previous = design_survey_index_previous %>% mutate(area_lab = 
                                                       case_when(area_lab == "AI" ~ "BS_AI_WGOA",
                                                                 area_lab == "BS" ~ "BS_AI_WGOA",
                                                                 area_lab == "WGOA" ~ "BS_AI_WGOA",
                                                                 TRUE ~ area_lab))
## sum estimates for BS, AI, WGOA
design_survey_index_previous = design_survey_index_previous %>% group_by(Year, area_lab) %>% summarise(sum_estimates = sum(sum_estimates), sum_var = sum(sum_var), se = sqrt(sum_var), LCI = sum_estimates - 2*se, UCI = sum_estimates + 2*se)
design_survey_index_previous$type = "previous"
design_survey_index_previous$Country = "Combined"
full_srv_abund = bind_rows(design_survey_index, design_survey_index_previous)

## compare the effect of splitting out Country
ggplot(full_srv_abund, aes(Year, sum_estimates, col = type, linetype = type, alpha = 0.3)) +
  geom_line(linewidth = 1) +
  labs(x = "Year", y = "Relative index (RPN)", linetype = "", col = "", fill = "") +
  guides(alpha = "none") +
  facet_wrap(~area_lab) +
...
scenario = paste0("1977_NoTagData_countrysrv", ifelse(AF_direct_ageing, "a", ""))
...
######################
scenario = paste0("1977_NoTagData_countrysrv_regionaldevs", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# Try estimate movement
######################
scenario = paste0("1977_NoTagData_est_move_rec_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("1977_fenske_movement_countrysrv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
## Add movement matrix from Fenske's dissertation
######################
scenario = paste0("1977_fenske_movement_spt_q_countrysrv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_TagData_countrysrv_01", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_TagData_01_countrysrv_alt", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
if(read_in_previous_model_data_and_params) {
  previous_scenario = paste0("1977_TagData_countrysrv_01", ifelse(AF_direct_ageing, "a", ""))
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_TagData_reg_rec_countrysrv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_TagData_decadal_countrysrv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_TagData_01a_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
## constrain recruitment to sum = 0
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_Model_02_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_Model_03_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_Model_04_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
if(T) {
  prev_scenario = paste0("1977_Model_03_srv", ifelse(AF_direct_ageing, "a", ""))
  prev_fig_path = file.path("Output", "3-Area", prev_scenario)
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_Model_05_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
## if starting from here re-load previous models data and parameters
if(T) {
  prev_scenario = paste0("1977_Model_04_srv", ifelse(AF_direct_ageing, "a", ""))
  prev_fig_path = file.path("Output", "3-Area", prev_scenario)
  data = readRDS(file.path(prev_fig_path, "data.RDS"))
  parameters = readRDS(file.path(prev_fig_path, "parameters.RDS"))
}
...
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_Model_06_srv", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path("Output", "3-Area", scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
## if starting from here re-load previous models data and parameters
if(T) {
  prev_scenario = paste0("1977_Model_05_srv", ifelse(AF_direct_ageing, "a", ""))
  prev_fig_path = file.path("Output", "3-Area", prev_scenario)
  data = readRDS(file.path(prev_fig_path, "data.RDS"))
  parameters = readRDS(file.path(prev_fig_path, "parameters.RDS"))
}
