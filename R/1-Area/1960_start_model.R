...
#' Differs from current assessemnt by having differently structured compositional data
#' no overlap of AF and LF, sex-aggregated. I don't have the early data CPUE LF data that is in the 
#' assessment model and also I have dropped the trawl survey ndx.
library(here)
library(data.table)
library(tidyverse)
...
AF_direct_ageing = T # whether to use direct ageing
...
# Restructure input smaple sizes to make sure they're not too large
...
# Read in design-based survey index
design_survey_index = readRDS(file = here("Data", "Survey", "survey_ndx_5_area_combined_countries.RDS"))
...
years = 1960:2021 # model years
min_age = 2 # minimum age
max_age = 31 # max age plus group
ages = min_age:max_age # vector of ages
length_bins = seq(from = 41, to = 99, by = 2)  # length bins
region_key = data.frame(area = c("SingleArea"), TMB_ndx = c(0)) # number of areas
M = 0.104884 # natural mortality
reg_lvls = region_key$area[region_key$TMB_ndx + 1] ## need to be in correct order
year_lvls = years ## need to be in correct order

## Ordering for Sex-Age-Length stuff (Males first, followed by females)
sex_length_lvls = paste0(rep(c("M","F"), each = length(length_bins)), length_bins) # length bin orders
sex_age_lvls = paste0(rep(c("M","F"), each = length(ages)), ages) # age bin orders
trawl_observer_LF$sex_length = factor(trawl_observer_LF$sex_length, levels = sex_length_lvls, ordered = T) # order bins
fixed_observer_LF$sex_length = factor(fixed_observer_LF$sex_length, levels = sex_length_lvls, ordered = T) # order bins
fixed_gear_AF_alk_pooled$sex_age = factor(fixed_gear_AF_alk_pooled$sex_age, levels = sex_age_lvls, ordered = T) # order bins
fixed_gear_AF_direct$sex_age = factor(fixed_gear_AF_direct$sex_age, levels = sex_age_lvls, ordered = T) # order bins
survery_AF_direct$sex_age = factor(survery_AF_direct$sex_age, levels = sex_age_lvls, ordered = T) # order bins 
survery_AF_alk_pooled$sex_age = factor(survery_AF_alk_pooled$sex_age, levels = sex_age_lvls, ordered = T) # order bins
...
data$F_iterations = 4
## 
data$prop_F_hist = 0 ## no initial F applied
...
data$F_method = 1 ## 0 = estimate free F's
...
plot_SSB(mle_report, region_key)
mle_report$Bzero
...
#' @param observation string defining type
#' \itemize{
#'   \item `fixed`: fixed gear fishery
#'   \item `trwl`: trawl gear fishery
#'   \item `srv_dom_ll`: survey observation
#' }
#' @param type string defining type
#' \itemize{
#'   \item `age`: Age frequency
#'   \item `length`: length frequency
#' }
#' 
get_aggregated_comp_fits <- function(MLE_report, region_key = NULL, observation = "fixed", type = "age") {
  if(!type %in% c("age", "length"))
    stop("type needs to be either 'trwl', or 'length'")
  if(!observation %in% c("fixed", "trwl", "srv_dom_ll"))
    stop("type needs to be either 'fixed', 'trwl', or 'srv_dom_ll'")

  this_observation = NULL
  likelihood = NULL
  if (type == "age") {
    ## get observed value
    this_observation = get_AF(MLE_report = MLE_report, observation = observation)
    ## calculate proportions
    this_observation = this_observation %>% group_by(Region, Year) %>% mutate(prop_obs = Observed / sum(Observed), prop_pred = Predicted / sum(Predicted), N = sum(Observed))
    this_observation$theta = 1
    if(observation == "fixed") {
      if(MLE_report$fixed_catchatage_comp_likelihood == 0) {
        ## multinomial likelihood
        likelihood = 0
      } else if(MLE_report$fixed_catchatage_comp_likelihood == 0) {
        ## Dirchlet-multinomial likelihood
        likelihood = 1
        this_observation$theta = MLE_report$theta_fixed_catchatage
      }
    } else if(observation == "srv_dom_ll") {
      if(MLE_report$srv_dom_ll_catchatage_comp_likelihood == 0) {
        ## multinomial likelihood
        likelihood = 0
      } else if(MLE_report$srv_dom_ll_catchatage_comp_likelihood == 0) {
        ## Dirchlet-multinomial likelihood
        likelihood = 1
        this_observation$theta = MLE_report$theta_srv_dom_ll_catchatage
      }
    }
    ## calculate residuals
    if(likelihood == 0) {
      this_observation$pearson_resids = (this_observation$prop_obs - this_observation$prop_pred)
