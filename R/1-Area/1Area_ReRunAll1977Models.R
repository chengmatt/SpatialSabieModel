#'
#' Re-run all 1-Area model permutations
...
parameters$trans_srv_q = array(log(7), dim =c(n_regions,max(apply(data$srv_q_by_year_indicator, 2,FUN = function(x){length(unique(x))})), data$n_surveys))
...
######################
## First model run 1977_start
## only F-init estiamted not init age-devs
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("1977_start", ifelse(AF_direct_ageing, "a", ""))
...
# profile R0
if(run_profile) {
  ln_R0_profile_pars = seq(from = 1.5, to = 3.5, length = 15)
  ...
  ln_F_init_profile_pars = log(seq(from = 0.01, to = 0.1, length = 10))
  profile_ln_Finit = profile_param(mle_obj = mle_obj, parameters = mle_param_list, na_map = map_fixed_pars, profile_param_label = "ln_init_F_avg", profile_values = ln_F_init_profile_pars)
  saveRDS(profile_ln_Finit, file.path(fig_path, "profile_ln_Finit.RDS"))
  
  profile_labs = get_multiple_nlls(profile_ln_Finit$profile_mle, run_labels = NULL, region_key)
  profile_labs$Finit = exp(ln_F_init_profile_pars[profile_labs$label])
  
  ggplot(profile_labs %>% group_by(observations) %>% mutate(standardised_nll = negloglike - min(negloglike)), aes(x = Finit, y = standardised_nll, col = observations, linetype = observations)) +
    geom_line(linewidth = 1.1) +
    coord_cartesian(ylim=c(0,100)) +
    geom_vline(xintercept = mle_report$init_F_hist, col = "red", linetype = "dashed", linewidth = 1.1) +
    theme_bw() +
    theme(axis.title = element_text(size = 14), 
          axis.text = element_text(size = 14))
  ggsave(filename = file.path(fig_path, "Finit_profile.png"), width = 8, height = 6)
  
}

png(filename = file.path(fig_path, "RecruitDevs.png"), width = 7, height = 6, res = 250, units = "in")
rec_density = density(mle_report$recruitment_devs)
hist(mle_report$recruitment_devs, probability = T, breaks = 20, xlab = "Recruitment deviations", main = "")
lines(rec_density$x, rec_density$y, col = "red", lty = 2, lwd =3)
rec_vals = seq(-3, 3, by = 0.01)
rec_theory = dnorm(rec_vals, 0, sd = mle_report$sigma_R)
lines(rec_vals, rec_theory, col = "blue", lty = 2, lwd =3)
legend('topright', legend = c("Estimated", "Estimated", "Prior"), col = c("gray", "red", "blue"), lty = c(1,2,2), lwd = 3)
dev.off()


######################
## Estimate init devs
## to see if that makes much of a difference.
######################
scenario = paste0("1977_init_devs", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
data$n_init_rec_devs = 8
parameters$ln_init_rec_dev = rep(0, data$n_init_rec_devs)
...
######################
scenario = paste0("1977_rec_sum_zero", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("1977_DM_comp_likelihoods", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
data$srv_catchatage_comp_likelihood = c(0,1,1) ## keep japaneses fishery AF as mulinomial no AF data for this survey
...
parameters$trans_srv_catchatage_error = rep(log(0.5), data$n_surveys)

## fix japanease-fishery survey AF theta parameter
#na_map = fix_pars(par_list = parameters, pars_to_exclude = "trans_srv_catchatage_error", vec_elements_to_exclude = list(trans_srv_catchatage_error = c(1)))
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
...
######################
scenario = paste0("1977_include_tag_data", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
## turn back to multinomial
data$fixed_catchatage_comp_likelihood = 0
data$fixed_catchatlgth_comp_likelihood = 0
data$srv_catchatage_comp_likelihood = rep(0,data$n_surveys)
data$trwl_catchatlgth_comp_likelihood = 0
...
######################
scenario = paste0("1977_negative_binomial", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("1977_decadal_tag_report", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(here("Output", "1-Area"), scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
