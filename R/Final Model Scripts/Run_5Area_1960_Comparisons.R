# Purpose: Run 5-Area model as a final model for comparison 1960
# Creator: Matthew LH. Cheng (UAF-CFOS)
# Date: 9/4/24

# Set up ------------------------------------------------------------------
library(data.table)
library(tidyverse)
library(RColorBrewer)
library(SpatialSablefishAssessment)
library(TMB)
library(here)
source(here("R", "Utility_Fxns.R"))

scenario = "1960_start" # Model scenario
fig_path = here("Figs", "5-Area-1960")
out_path = here("Output", "Final Models", "5-Area-1960")

# Load in data and parameters
data = readRDS(file = here(out_path, "data.RDS"))
parameters = readRDS(file = here(out_path, "parameters.RDS"))
multiple_shoot = TRUE # whether to do multiple shooting
shoot_iter = 5 # number of iterations to multiple shoot for

# Run Models ---------------------------------------------------------------

### Poisson -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Poisson
# Movement is estimated 
# Spatially-invariant q
# Constant reporting rates

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-01-Poisson")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F 
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "constant" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement (time-invariant)
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 0

# Map parameters off
na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data, 
                                   parameters = parameters,
                                   na_map = na_map,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    } 
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                         gradient  = mle_obj$gr, 
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break 
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                       gradient  = mle_obj$gr, 
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

### Negative Binomial -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Negative Binomial
# Movement is estimated (time-invariant)
# Spatially-invariant q
# Constant reporting rates

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-01-NegBin")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F 
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "constant" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement (time-invariant)
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data, 
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    } 
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                         gradient  = mle_obj$gr, 
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break 
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                       gradient  = mle_obj$gr, 
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

### Negative Binomial (Reporting = Recent Block) -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Negative Binomial
# Movement is estimated 
# Spatially-invariant q
# Reporting rates in 2 blocks (1960 - 2016, 2016 - terminal)

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-02-Recent")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F 
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = c(1978, 2017) # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement (time-invariant)
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data, 
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    } 
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                         gradient  = mle_obj$gr, 
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break 
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                       gradient  = mle_obj$gr, 
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

### Negative Binomial (Reporting = Decadal) -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Negative Binomial
# Movement is estimated 
# Spatially-invariant q
# Decadal reporting rates

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-02-Decadal")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F 
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = c(1978, 1990, 2000, 2010) # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement (time-invariant)
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data, 
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    } 
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                         gradient  = mle_obj$gr, 
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break 
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                       gradient  = mle_obj$gr, 
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

### Negative Binomial (Reporting = Spatial) -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Negative Binomial
# Movement is estimated 
# Spatially-invariant q
# Decadal reporting rates

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-02-Space")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F 
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "space" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement (time-invariant)
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data, 
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    } 
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                         gradient  = mle_obj$gr, 
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break 
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn, 
                       gradient  = mle_obj$gr, 
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

### Negative Binomial (Time-Varying Movement) -----------------------------------------------------------------
# Parameterization
# Start year 1960
# 5 regions, 2 fishery fleets, and 2 survey fleets
# Mean recruitment regional
# Estimate all regional recruitment deviations except for the last year (does not sum to zero)
# Time block for fixed gear fleet selectivity from 1960 - 2015, 2016 - terminal (logistic)
# Time invariant trawl fishery selectivity (gamma)
# Selectivity for fixed gear deltas and trawl are shared across sexes
# Tag data used, Negative Binomial
# Movement is estimated (time-varying)
# Spatially-constant q
# Constant reporting rates

#### 4 Time Blocks -----------------------------------------------------------

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-03-Time_4_Move")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "constant" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Set up time-varying movement (data inputs)
data$n_movement_time_blocks = 4 # 4 movement blocks (fixed)
data$apply_fixed_movement = 0 # estimated movement matrix
data$fixed_movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # fixed movement
data$movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # estiamted movement matrix
data$movement_time_block_indicator = c(rep(0, length(1960:1990)), rep(1, length(1991:2000)),
                                       rep(2, length(2001:2010)), rep(3, length(2011:2021)))
movement_matrix = fixed_movement_matrix = matrix(0, nrow = data$n_regions, ncol = data$n_regions)
diag(fixed_movement_matrix) = 1 # set 1 for fixed movement matrix
diag(movement_matrix) = 0.9 # set 0.9 for estimated movement matrix (cant be 1)
# data$fixed_movement_matrix[,,1] = fixed_movement_matrix # define movement fixed 1 region
data$fixed_movement_matrix[,,] = fixed_movement_matrix # define movement fixed 1 region
movement_matrix = movement_matrix + rlnorm(n = data$n_regions * data$n_regions, log(0.01), 0.1) # random deviate here for 0s in movement matrix
# Normalize so rows sum to 1 in estimated movement matrix
movement_matrix = sweep(movement_matrix, 1, STATS = rowSums(movement_matrix), "/")
data$movement_matrix[] = movement_matrix

# Set up time-varying movement (parameter inputs)
parameters$transformed_movement_pars = array(NA, dim = c(data$n_regions - 1, data$n_regions, data$n_movement_time_blocks))
for(i in 1:data$n_regions) parameters$transformed_movement_pars[,i,] = simplex(movement_matrix[i,])
validate_input_data_and_parameters(data, parameters)

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data,
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    }

    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)

    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                         gradient  = mle_obj$gr,
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged

    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})

    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break

  } # end i
} else {

  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)

  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                       gradient  = mle_obj$gr,
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged

  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})

  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report

}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

#### 3 Time Blocks -----------------------------------------------------------

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-03-Time_3_Move")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "constant" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Set up time-varying movement (data inputs)
data$n_movement_time_blocks = 3 # 3 movement blocks (fixed)
data$apply_fixed_movement = 0 # estimated movement matrix
data$fixed_movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # fixed movement
data$movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # estiamted movement matrix
data$movement_time_block_indicator = c(rep(0, length(1960:1990)), rep(1, length(1991:2005)), rep(2, length(2006:2021)))
movement_matrix = fixed_movement_matrix = matrix(0, nrow = data$n_regions, ncol = data$n_regions)
diag(fixed_movement_matrix) = 1 # set 1 for fixed movement matrix
diag(movement_matrix) = 0.9 # set 0.9 for estimated movement matrix (cant be 1)
# data$fixed_movement_matrix[,,1] = fixed_movement_matrix # define movement fixed 1 region
data$fixed_movement_matrix[,,] = fixed_movement_matrix # define movement fixed 1 region
movement_matrix = movement_matrix + rlnorm(n = data$n_regions * data$n_regions, log(0.01), 0.1) # random deviate here for 0s in movement matrix
# Normalize so rows sum to 1 in estimated movement matrix
movement_matrix = sweep(movement_matrix, 1, STATS = rowSums(movement_matrix), "/")
data$movement_matrix[] = movement_matrix

# Set up time-varying movement (parameter inputs)
parameters$transformed_movement_pars = array(NA, dim = c(data$n_regions - 1, data$n_regions, data$n_movement_time_blocks))
for(i in 1:data$n_regions) parameters$transformed_movement_pars[,i,] = simplex(movement_matrix[i,])
validate_input_data_and_parameters(data, parameters)

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data,
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    }

    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)

    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                         gradient  = mle_obj$gr,
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged

    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})

    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break

  } # end i
} else {

  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)

  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                       gradient  = mle_obj$gr,
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged

  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})

  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report

}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))

#### 2 Time Blocks -----------------------------------------------------------

# Set up path for outputting model
model_path = here("Output", "Final Models", "5-Area-1960", "5-Area-1960-03-Time_2_Move")
dir.create(model_path)

srv_sel_first_param_shared_by_sex = F
srv_sel_second_param_shared_by_sex = T
fixed_sel_first_param_shared_by_sex  = F
fixed_sel_second_param_shared_by_sex   = T
trwl_sel_first_param_shared_by_sex  = F
trwl_sel_second_param_shared_by_sex  = T
recruit_dev_years_not_to_estimate = 2020:2021 # don't estimate rec devs for last 2 years
srv_q_spatial = F # spatial q
est_init_F = F # estimate initial F
tag_reporting_rate = "constant" # no tag reporting
est_catch_sd = F # fixed catch sd
est_movement = T # estimated movement
est_prop_male_recruit = "off" # fixed sex-ratio
data$tag_likelihood = 1

# Set up time-varying movement (data inputs)
data$n_movement_time_blocks = 2 # 2 movement blocks (fixed)
data$apply_fixed_movement = 0 # estimated movement matrix
data$fixed_movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # fixed movement
data$movement_matrix = array(0, dim = c(data$n_regions, data$n_regions, data$n_movement_time_blocks)) # estiamted movement matrix
data$movement_time_block_indicator = c(rep(0, length(1960:1990)), rep(1, length(1991:2021)))
movement_matrix = fixed_movement_matrix = matrix(0, nrow = data$n_regions, ncol = data$n_regions)
diag(fixed_movement_matrix) = 1 # set 1 for fixed movement matrix
diag(movement_matrix) = 0.9 # set 0.9 for estimated movement matrix (cant be 1)
# data$fixed_movement_matrix[,,1] = fixed_movement_matrix # define movement fixed 1 region
data$fixed_movement_matrix[,,] = fixed_movement_matrix # define movement fixed 1 region
movement_matrix = movement_matrix + rlnorm(n = data$n_regions * data$n_regions, log(0.01), 0.1) # random deviate here for 0s in movement matrix
# Normalize so rows sum to 1 in estimated movement matrix
movement_matrix = sweep(movement_matrix, 1, STATS = rowSums(movement_matrix), "/")
data$movement_matrix[] = movement_matrix

# Set up time-varying movement (parameter inputs)
parameters$transformed_movement_pars = array(NA, dim = c(data$n_regions - 1, data$n_regions, data$n_movement_time_blocks))
for(i in 1:data$n_regions) parameters$transformed_movement_pars[,i,] = simplex(movement_matrix[i,])
validate_input_data_and_parameters(data, parameters)

# Map parameters off
# na_map = fix_pars(par_list = parameters, pars_to_exclude = "ln_tag_phi") # fix negative binomial parameter
map_fixed_pars = set_up_parameters(data = data,
                                   parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
                                   fixed_sel_first_shared_by_sex  = fixed_sel_first_param_shared_by_sex,
                                   fixed_sel_second_shared_by_sex   = fixed_sel_second_param_shared_by_sex,
                                   trwl_sel_first_shared_by_sex  = trwl_sel_first_param_shared_by_sex,
                                   trwl_sel_second_shared_by_sex  = trwl_sel_second_param_shared_by_sex,
                                   recruit_dev_years_not_to_estimate = recruit_dev_years_not_to_estimate,
                                   srv_q_spatial = srv_q_spatial,
                                   tag_reporting_rate = tag_reporting_rate,
                                   est_init_F = est_init_F,
                                   est_catch_sd = est_catch_sd,
                                   est_movement = est_movement,
                                   est_prop_male_recruit = est_prop_male_recruit)

if(multiple_shoot == TRUE) {
  for(i in 1:shoot_iter) {
    if(i > 1) {
      parameters$trans_rec_dev[] = mle_report$recruitment_devs
      parameters$ln_init_rec_dev[] = log(mle_report$init_rec_dev)
      parameters$ln_tag_phi = log(mle_report$tag_phi)
      parameters$ln_trwl_sel_pars[] = log(mle_report$trwl_sel_pars)
      parameters$ln_srv_sel_pars[] = log(mle_report$srv_sel_pars)
      parameters$trans_srv_q[] = log(mle_report$srv_q)
      parameters$logistic_tag_reporting_rate[] = logit(mle_report$tag_reporting_rate)
    }
    
    mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
    pre_optim_sanity_checks(mle_obj)
    
    mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                         gradient  = mle_obj$gr,
                         control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
    mle_spatial$convergence # check convergence, 0 = converged
    
    # Run more newton steps
    try_improve = tryCatch(expr =
                             for(i in 1:3) {
                               g = as.numeric(mle_obj$gr(mle_spatial$par))
                               h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                               mle_spatial$par = mle_spatial$par - solve(h,g)
                               mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                             }, error = function(e){e})
    
    # check post optimization
    post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_obj$env$last.par.best)
    mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
    if(max(abs(mle_obj$gr(mle_obj$env$last.par.best))) < 0.001) break
    
  } # end i
} else {
  
  mle_obj = MakeADFun(data, parameters, map = map_fixed_pars, DLL="SpatialSablefishAssessment_TMBExports", hessian = T)
  pre_optim_sanity_checks(mle_obj)
  
  mle_spatial = nlminb(start = mle_obj$par, objective = mle_obj$fn,
                       gradient  = mle_obj$gr,
                       control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  mle_spatial$convergence # check convergence, 0 = converged
  
  # Run more newton steps
  try_improve = tryCatch(expr =
                           for(i in 1:3) {
                             g = as.numeric(mle_obj$gr(mle_spatial$par))
                             h = optimHess(mle_spatial$par, fn = mle_obj$fn, gr = mle_obj$gr)
                             mle_spatial$par = mle_spatial$par - solve(h,g)
                             mle_spatial$objective = mle_obj$fn(mle_spatial$par)
                           }, error = function(e){e})
  
  # check post optimization
  post_optim_sanity_checks(mle_obj = mle_obj, mle_pars = mle_spatial$par)
  mle_report = mle_obj$report(mle_obj$env$last.par.best) # get report
  
}

sd_report = sdreport(mle_obj)

# Save stuff
saveRDS(data, file.path(model_path, "data.RDS"))
saveRDS(parameters, file.path(model_path, "parameters.RDS"))
saveRDS(mle_report, file.path(model_path, "mle_report.RDS"))
saveRDS(sd_report, file.path(model_path, "sd_report.RDS"))
saveRDS(mle_spatial, file.path(model_path, "mle_optim.RDS"))
saveRDS(map_fixed_pars, file.path(model_path, "map_fixed_pars.RDS"))
mle_param_list = mle_obj$env$parList(par = mle_obj$env$last.par.best)
saveRDS(mle_param_list, file.path(model_path, "mle_par_list.RDS"))
