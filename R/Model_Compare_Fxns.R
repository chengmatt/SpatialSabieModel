...
#   
#   # Fixed gear
#   fixed_f = reshape2::melt(model_list[[i]]$sel_fixed_f) %>% mutate(model = model_name[i], type = paste("Fixed Gear Fishery F", Var2))
#   fixed_m = reshape2::melt(model_list[[i]]$sel_fixed_m) %>% mutate(model = model_name[i], type = paste("Fixed Gear Fishery M", Var2))
#   
#   # Survey gear (dom)
...
#   # Get array
#   # females
#   sel_fixed_f = aperm(replicate(model_dat[[i]]$n_regions, cbind(replicate(length(1960:2015), area5$sel_fixed_f[,1]),
#                                          replicate(length(2016:2022), area5$sel_fixed_f[,2]))), c(1,3,2))
#   
#   # males
#   sel_fixed_m = aperm(replicate(model_dat[[i]]$n_regions, cbind(replicate(length(1960:2015), area5$sel_fixed_m[,1]),
#                                          replicate(length(2016:2022), area5$sel_fixed_m[,2]))), c(1,3,2))
#   
#   # Get exploitable biomass for fixed gear at age
#   expl_fixed_age = sel_fixed_f * model_list[[i]]$natage_f * waa_f +
#                    sel_fixed_m * model_list[[i]]$natage_m * waa_m 
# 
#   
