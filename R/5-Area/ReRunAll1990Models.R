...
max_N_eff = 500 ## during the initial input calculations some of the input sample sizes can be quite large, which I don't want effecting the model
min_N_eff = 50
N_eff_multiplier = 0.4 ## only relevant for mult


## read in observational datasets
fixed_gear_AF_alk_pooled = readRDS(file = file.path(DIR$data5A, "Observer_ALK_AF_w_eff.RDS"))
fixed_gear_AF_direct = readRDS(file = file.path(DIR$data5A, "Observer_direct_AF_w_eff.RDS"))
trawl_observer_LF = readRDS(file = file.path(DIR$data5A, "Observer_trawl_LF_w_eff.RDS"))
fixed_observer_LF = readRDS(file = file.path(DIR$data5A, "Observer_fixed_LF_w_eff.RDS"))
survery_AF_direct = readRDS(file = file.path(DIR$data5A, "Survey_direct_AF_w_eff.RDS"))
survery_AF_alk_pooled = readRDS(file = file.path(DIR$data5A, "Survey_ALK_AF_w_eff.RDS"))
...
## Tag data
tag_recovery_df = readRDS(file = file.path(DIR$data5A, "Tag_recovery_summarised.RDS"))
tag_release_df = readRDS(file = file.path(DIR$data5A, "Tag_release_summarised.RDS"))

## read in Catch
full_catch_df = readRDS(file = file.path(DIR$data5A, "Catch_by_year_area_gear.RDS"))

fixed_gear_with_imputation = readRDS(file = file.path(DIR$data5A, "fixed_gear_with_imputations_S1.RDS"))
trawl_gear_with_imputation = readRDS(file = file.path(DIR$data5A, "trawl_gear_with_imputations_S1.RDS"))
...
data$male_age_length_transition[,,as.character(1990:1994)] = sab_inputs$male_length_at_age_60_96
data$female_age_length_transition[,,as.character(1990:1994)] = sab_inputs$female_length_at_age_60_96
...
## replace zeros with a low number to avoid log(0)
fixed_catch_real[fixed_catch_real == 0] = 0.001
trwl_catch_real[trwl_catch_real == 0] = 0.001

data$fixed_fishery_catch = as.matrix(fixed_catch_real)
data$trwl_fishery_catch =  as.matrix(trwl_catch_real)
...
data$srv_sel_by_year_indicator[projyears > 1994, 1] =1

#'
#' Tag release stuff
#'
tag_release_years = c(1990, 1991, 1992, 1993, 1994, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016)
...
data$srv_catchatage_indicator = array(as.matrix(survey_AF_indicator %>% dplyr::select(-region_f), nrow = n_regions, ncol = n_years), dim = c(n_regions, n_years,data$n_surveys))
...
### Survey LL index
data$srv_bio_indicator = array(as.matrix(survey_index_indicator %>% dplyr::select(-region_f), nrow = n_regions, ncol = n_years), dim = c(n_regions, n_years, data$n_surveys))
...
data$srv_q_by_year_indicator[projyears > 1994, 1] = 1
...
parameters$ln_srv_sel_pars = array(0, dim = c(length(unique(data$srv_sel_by_year_indicator )), 2, 2, data$n_surveys))
...
if(dim(parameters$ln_srv_sel_pars)[1] == 1) {
  ...
  parameters$ln_srv_sel_pars[1,1,1,1] =  parameters$ln_srv_sel_pars[2,1,1,1] = suppressWarnings(as.numeric(unlist(strsplit(sab_par[grep("log_a50_srv1_m:",sab_par)+1],split=" "))))
  parameters$ln_srv_sel_pars[1,2,1,1] = parameters$ln_srv_sel_pars[2,2,1,1] = suppressWarnings(as.numeric(unlist(strsplit(sab_par[grep("log_delta_srv1_m:",sab_par)+1],split=" "))))
  parameters$ln_srv_sel_pars[1,1,2,1] = parameters$ln_srv_sel_pars[2,1,2,1] = suppressWarnings(as.numeric(unlist(strsplit(sab_par[grep("log_a50_srv1_f:",sab_par)+1],split=" "))))
  parameters$ln_srv_sel_pars[1,2,2,1] = parameters$ln_srv_sel_pars[2,2,2,1] = suppressWarnings(as.numeric(unlist(strsplit(sab_par[grep("log_delta_srv1_f:",sab_par)+1],split=" "))))
...
parameters$trans_srv_q = array(log(7), dim =c(n_regions, length(unique(data$srv_q_by_year_indicator)), data$n_surveys))
...
parameters$trans_srv_catchatage_error = log(1)
...
######################
## First model run Model1990_01
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_01", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
...
BS_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:200, release_region_to_plt = "BS")
BS_releases
ggsave(filename = file.path(fig_path, "BS_tag_release_and_recovery.png"), width = 7, height = 7)
AI_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:200, release_region_to_plt = "AI")
AI_releases
ggsave(filename = file.path(fig_path, "AI_tag_release_and_recovery.png"), width = 7, height = 7)
WGOA_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:200, release_region_to_plt = "WGOA")
WGOA_releases
ggsave(filename = file.path(fig_path, "WGOA_tag_release_and_recovery.png"), width = 12, height = 8)
CGOA_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:200, release_region_to_plt = "CGOA")
CGOA_releases
ggsave(filename = file.path(fig_path, "CGOA_tag_release_and_recovery.png"), width = 12, height = 8)
EGOA_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:200, release_region_to_plt = "EGOA")
EGOA_releases
ggsave(filename = file.path(fig_path, "EGOA_tag_release_and_recovery.png"), width = 12, height = 8)

tag_report_time_block = c(1991, 2000, 2010)
rec_devs_not_to_estimate = NULL#1960:1976
...
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   ...
                                   recruit_dev_years_not_to_estimate = rec_devs_not_to_estimate,  ## don't estimate the last one
...
######################
## First model run Model1990_02
## Est rec devs back to 1970
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_02", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
#rec_devs_not_to_estimate = 1960:1969
...
######################
## First model run Model1960_03
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_03", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
validate_input_data_and_parameters(data, parameters)
# the first time A ran this model. I ran into convergence issues.
# the D-M parameter for fixed LF went to upper bound
# also the D-M parameter for survey AF was large (approx 100) which seems unreasonable.
# I am fixeing the D-M parameter for fixed LF at 0.5 to see if that works
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
...
######################
## First model run Model1960_04
## run model back to 1960 change
## historic catch allocation S2
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_04", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}

fixed_gear_with_imputation = readRDS(file = file.path(DIR$data5A, "fixed_gear_with_imputations_S2.RDS"))
trawl_gear_with_imputation = readRDS(file = file.path(DIR$data5A, "trawl_gear_with_imputations_S2.RDS"))
...
## change Rec devs back to not to sum to zero
data$rec_devs_sum_to_zero = 0
if(data$rec_devs_sum_to_zero == 1) {
  if(data$global_rec_devs == 1) {
    parameters$trans_rec_dev = matrix(0, nrow = 1, ncol = n_years - 1)
...
# also the D-M parameter for survey AF was large (approx 100) which seems unreasonable.
# I am fixeing the D-M parameter for fixed LF at 0.5 to see if that works
rec_devs_not_to_estimate = 1960:1976
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   na_map = NULL,
                                   srv_sel_first_param_shared_by_sex = srv_sel_first_param_shared_by_sex,
                                   srv_sel_second_param_shared_by_sex = srv_sel_second_param_shared_by_sex,
...
######################
## First model run Model1960_05
## Est rec devs back to 1970 with historic S2 catch allocation
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_05", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
## First model run Model1960_06
## Est rec devs back to 1970 (sum to zero) with historic S2 catch allocation
######################
# options(warn=0) ## if warnings are being set to errors
scenario = paste0("Model1990_06", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
