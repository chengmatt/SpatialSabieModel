...
parameters$trans_srv_q = array(log(1000), dim =c(n_regions, length(unique(data$srv_q_by_year_indicator)), data$n_surveys))
...
scenario = paste0("Model_01", ifelse(AF_direct_ageing, "a", ""))
...
BS_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:90, release_region_to_plt = "BS")
BS_releases
ggsave(filename = file.path(fig_path, "BS_tag_release_and_recovery.png"), width = 7, height = 7)
AI_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:90, release_region_to_plt = "AI")
AI_releases
ggsave(filename = file.path(fig_path, "AI_tag_release_and_recovery.png"), width = 7, height = 7)
WGOA_releases = plot_frequency_of_tag_release_and_recoveries(data, region_key = region_key, release_ndx_to_plot = 1:90, release_region_to_plt = "WGOA")
WGOA_releases
...
######################
scenario = paste0("Model_02", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
parameters$trans_srv_catchatage_error = as.vector(parameters$trans_srv_catchatage_error)
est_fixed_LF_theta = F # these two D-M parameters were running to upper bounds
est_srv_AF_theta = F
...
map_fixed_pars = set_up_parameters(data = data, parameters = parameters,
                                   ...
                                   est_fixed_LF_theta = est_fixed_LF_theta,
                                   est_trwl_LF_theta = T,
                                   est_srv_AF_theta = est_srv_AF_theta
                                   #est_prop_male_recruit = seq(from = 1990, to = 2020, by = 10)
)
...
######################
scenario = paste0("Model_03", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("Model_04", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("Model_05", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("Model_06", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
######################
scenario = paste0("Model_07", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
...
if(read_in_previous_model_data_and_params) {
  previous_scenario = paste0("Model_06", ifelse(AF_direct_ageing, "a", ""))
  previous_fig_path = file.path(DIR$app_5A, previous_scenario)
...
######################
## Run model run 08
## Change trawl selectivity 
######################
scenario = paste0("Model_08", ifelse(AF_direct_ageing, "a", ""))
fig_path = file.path(DIR$app_5A, scenario)
if(!dir.exists(fig_path)) {
  dir.create(fig_path)
}
